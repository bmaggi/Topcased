...
public class InsertTableColumnAction extends RichTextAction {
	...
	public InsertTableColumnAction(IRichText richText) {
        ...
        setImageDescriptor(RichTextImages.IMG_DESC_INSERT_TABLE_COL);
        setToolTipText(RichTextResources.insertTableColAction_toolTipText);
	}
	...
	public void execute(IRichText richText) {
		if (richText != null) {
			...
			InputDialog dialog = new InputDialog(parent, RichTextResources.insertTableColumn_title, RichTextResources.insertTableColumn_text, "0", new NumberValidator()); //$NON-NLS-1$
			if (dialog.open() == Window.OK) {
				richText.executeCommand(RichTextCommand.INSERT_TABLE_COL, dialog.getValue());
			}
		}
	}
}