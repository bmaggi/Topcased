...
// Sets the background color of the selected text in Mozilla.
function mozillaBackColor(color) {
	if (color != null && color != '') {
...
//-------------------------------------------------------------------------------
// Provide Insert/Delete a Row/Column capabilities when a table is selected in IE

// Retrieve the selected table in the Browser
function getTable() {

	if(selection != null && selection.type.toLowerCase() == 'control') {
		var selectedElement;
		var selAnchorOffset = selection.anchorOffset;
		var selAnchorNode = selection.anchorNode;
		
		if (document.all) {
			selectedElement = selection.createRange().item(0); 
	 	} else if (selAnchorNode.childNodes.length > selAnchorOffset && selAnchorNode.childNodes[selAnchorOffset].nodeType == 1) {
	    	selectedElement = selAnchorNode.childNodes[selAnchorOffset];
	    } else if(selAnchorNode.nodeType == 1) {
	        selectedElement=  selAnchorNode;
	    } else {
	        selectedElement = selAnchorNode.parentNode;
	    } 
		
		if (selectedElement.nodeName.toLowerCase() == "table" ) {
...
function insertRow(index) {
	...
	var selectedElement = getTable();
	if(selectedElement != null) {
		var selAnchorOffset = selection.anchorOffset;
		var selAnchorNode = selection.anchorNode;
		
    	index = parseInt(index);
    	var rows = selectedElement.rows.length;
    	if (index >= rows) index = rows;
    	var newRow = selectedElement.insertRow(index);
    	var pos = index - 1;
		if (index == 0) 
  			pos = 1;
  
    	for (var i = 0; i < selectedElement.rows[pos].cells.length; i++) {
			newRow.insertCell(i);
		}
	}
}
...
function deleteRow(index) {
	...
	if(selectedElement != null) {
    	...
    	selectedElement.deleteRow(index);
	}
}
...
function insertColumn(index) {
	var selectedElement = getTable();
	if(selectedElement != null) {
    	var theTableBody = selectedElement.tBodies[0]
    	index = parseInt(index);
    	var cols = theTableBody.rows[0].cells.length;
    	if (cols <= index) {
    		index = cols;
    	}
    	if (selectedElement.tHead) {
	        var oneRow = selectedElement.tHead.rows[0]
	        oneRow.insertCell(index)
	    }
	    for (var i = 0; i < theTableBody.rows.length; i++) {
	        var oneRow = theTableBody.rows[i]
	        oneRow.insertCell(index)
	    }
	}
}
