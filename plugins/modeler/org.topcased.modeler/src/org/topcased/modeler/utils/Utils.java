...
public final class Utils
{
    ...
    public static Font getFont(FontData fontData)
    {
        if (fontData != null)
        {
        	// QuickFix due to the fact that fontData coming from the preference are not well collected.
        	fontData.setName(fontData.getName().replaceAll(";", ""));
     ...
     * @param modelElement the eAnnotation owner
     */
    public static void createAuthorEAnnotation(EModelElement modelElement)
    {
        
        EAnnotation annotation = EcoreFactory.eINSTANCE.createEAnnotation();
        annotation.setSource(IAnnotationConstants.AUTHOR_SOURCE);
        modelElement.getEAnnotations().add(annotation);
        String author = Utils.getPreferenceStoreAccordingToCurrentIFile().getString(ModelerPreferenceConstants.P_DEFAULT_AUTHOR);
        annotation.getDetails().put(IAnnotationConstants.AUTHOR_KEY, author);
    }
    ...
    public static Modeler getCurrentModeler()
    {
        ...
        if (workbench != null)
        {
            ...
            {
                // FIXME if windows > 1
                activeWorkbenchWindow = workbench.getWorkbenchWindows().length == 1 ? workbench.getWorkbenchWindows()[0] : null ;
            }
            if (activeWorkbenchWindow != null)
            {
    ...
    /**
     * Returns the good preference store according to variable currentIFile in Modeler If there is a problem, returns
