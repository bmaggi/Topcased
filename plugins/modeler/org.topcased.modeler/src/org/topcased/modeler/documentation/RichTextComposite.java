            ...
            try
            {
				...
				if (protocol2 != null)
                {
					...
					if (protocol2.equalsIgnoreCase(HTTP_PROTOCOL)) {
						this.uri = URI.createURI(url.replace("http://platform", "platform:").replace("platform::/", "platform:/"));//.replaceAll("%20", " "));
					}
					else if (protocol2.equalsIgnoreCase(PLATFORM_PROTOCOL)) {
						this.uri = URI.createURI(url.replaceAll("%20", " "));
					}
                }
            } 
            ...
            BusyIndicator.showWhile(display, new Runnable()
            {
                public void run()
                {
            		...
            		if (activeEditor instanceof Modeler)
            		{
            			...
            			EObject eObject = modeler.getResourceSet().getEObject(uri, false);
