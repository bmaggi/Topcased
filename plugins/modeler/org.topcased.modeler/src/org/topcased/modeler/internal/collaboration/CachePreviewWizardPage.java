...
class CachePreviewWizardPage extends WizardPage
{
    ...
    {
        ModelElementChooserDialog dlg = new ModelElementChooserDialog(getShell(), "Select element", "Select elements to add to the cache:");
        dlg.setAdapterFactory(adapterFactory);
        dlg.setInput(cacheSpec.getRootElement().eContainer().eResource());
        List<EObject> userElements = dlg.openMulti();
        boolean continueProcess = true ;
        for (EObject e : userElements)
        {
            if (!cacheSpec.isValidUserElement(e))
            {
