...
public class GenericTransformer
{
    ...
    private MixedEditDomain mixedDomain;
    ...
    {
        globalCommand = new CompoundCommand("Generic Transformation")
        {
            @Override
        ...
        if (editor instanceof Modeler)
        {
            ...
            mixedDomain = (MixedEditDomain) modeler.getAdapter(MixedEditDomain.class);
            ...
            if (globalCommand.canExecute())
            {
                try
                {
                    ...
                    mixedDomain.getCommandStack().execute(globalCommand);
                }
                catch (Exception e)
                {
     ...
     */
    private class ImporterCommand extends org.topcased.modeler.commands.CompoundCommand
    {
        ...
        public void execute()
        {
            Command c = mixedDomain.getCommandStack().getUndoCommand();
            graphCopy(mixedDomain, commandModel.getResultEobject());
            Command c2 = mixedDomain.getCommandStack().getUndoCommand();
            canUndo = c != c2;
        }
        ...
        {
            super.redo();
            mixedDomain.getCommandStack().redo();
        }
        ...
        public void undo()
        {
            try
            {
                ...
                if (canUndo)
                {
                    mixedDomain.getCommandStack().undo();
                }
            }
            catch (Exception e)
            {
         ...
         */
        private void graphCopy(MixedEditDomain domain, EObject target)
        {
            ...
            for (Diagram diagram : map.keySet())
            {
                ...
                importer.setCommandStack(domain.getCommandStack());
        ...
        // private void deleteGraphEdges(GraphElement graphElement)
        // {
        // List<Command> commands = new LinkedList<Command>();
        // EList<GraphConnector> anchor = graphElement.getAnchorage();
        // for (GraphConnector c : anchor)
        // {
        // EList<GraphEdge> edges = c.getGraphEdge();
        // for (GraphEdge e : edges)
        // {
        // DeleteGraphElementCommand command = new DeleteGraphElementCommand(e,false);
