...
public abstract class DiagramsPage extends WizardPage
{
    ...
    {
        ContainerSelectionDialog dialog = new ContainerSelectionDialog(getShell(), (IContainer) ModelerPlugin.getWorkspace().getRoot().findMember(directoryFd.getText()), false,
    ...
    private boolean validateExistingModel()
    {
        ...
        if (modelFd.getText() == null || "".equals(modelFd.getText()))
        {
            setErrorMessage("You have to choose an existing model file.");
            return false;
        }

        if (!((IFile) ModelerPlugin.getWorkspace().getRoot().findMember(modelFd.getText())).getFileExtension().equals(getFileExtension()))
        {
    ...
    private boolean validateNewModelGroup()
    {
        ...
        else if (getTemplateId() == null && (getDiagramId()) == null)
        {
    ...
    private boolean modelFileExist()
    {
        if (isNewModel())
        {
            String path = directoryFd.getText() + File.separator + getModelNameWithoutExtension(nameFd.getText()) + "." + getFileExtension();
            IResource resource = ModelerPlugin.getWorkspace().getRoot().findMember(path);
            return resource != null;
        }
        else
        {
            IResource resource = ModelerPlugin.getWorkspace().getRoot().findMember(modelFd.getText());
            return resource != null;
        }
    }
    ...
    private boolean diagramFileExist()
    {
        if (isNewModel())
        {
            String path = directoryFd.getText() + File.separator + getModelNameWithoutExtension(nameFd.getText()) + "." + getFileExtension() + "di";
            IResource resource = ModelerPlugin.getWorkspace().getRoot().findMember(path);
            return resource != null;
        }
        else
        {
            IResource resource = ModelerPlugin.getWorkspace().getRoot().findMember(modelFd.getText() + "di");
            return resource != null;
        }
    }
    ...
    public IContainer getSelectedIContainer()
    {
        if (isNewModel())
        {
            return (IContainer) ModelerPlugin.getWorkspace().getRoot().findMember(directoryFd.getText());
        }
        else
        {
            return ((IFile) ModelerPlugin.getWorkspace().getRoot().findMember(modelFd.getText())).getParent();
        }
    }
    ...
    public String getModelName()
    {
        ...
        else
        {
            ...
            {
                IResource resource = ModelerPlugin.getWorkspace().getRoot().findMember(modelFd.getText());
                return getModelNameWithoutExtension(resource.getName());
            }
        }
        return null;
    }
