...
/**
 * An abstract implementation of a section in a tab in the tabbed property sheet page for a tree editor. Clients
 * specializing this class may implement the create widgets, setFormData and hooklisteners rather than overriding the
 * create controls.
 * 
...
public abstract class AbstractTabbedPropertySection extends AbstractPropertySection
{
    ...
    /**
     * Get the standard label width when labels for sections line up on the left hand side of the composite. We line up
     * to a fixed position, but if a string is wider than the fixed position, then we use that widest string.
     * 
    ...
    /**
     * Manages a self-contained set of interrelated EMF models and the {@link Command}s that modify them. The models are
     * maintained in the form of a {@link ResourceSet}. Commands that modify the model are typically created through the
     * domain and are executed using the {@link CommandStack}.
     * 
    ...
    /**
     * Determine if the component is read only
     * 
     * @param enabled
     */
    ...
    /**
     * Section widgets should be created inside this method. To set the layout data please implement the
     * {@link #setSectionData(Composite)} If widgets has listeners implement the {@link #hookListeners()}.
     * 
    ...
    /**
     * This method should be implemented to set layout data to the widgets created at {@link #createWidgets(Composite)}.
     * This improves visibility
     * 
     * @param composite Sometimes widgets will be set formDatas and position will depend on parent composite.
     */
    ...
    /**
     * Widgets created at {@link #createWidgets(Composite)} may listen platform events. Set them here in order to
     * improve visibility
     */
