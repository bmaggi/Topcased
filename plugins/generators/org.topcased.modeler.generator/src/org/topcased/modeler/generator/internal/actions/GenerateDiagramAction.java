...
/**
 * Action that generates diagram classes conforming to a default JET templates.
 * 
 * @author <a href="mailto:jacques.lescot@anyware-tech.com">Jacques LESCOT</a>
 * @author <a href="mailto:david.sciamma@anyware-tech.com">David Sciamma</a>
 */
public class GenerateDiagramAction implements IActionDelegate
{
    ...
    public void run(IAction action)
    {
        ...
        WorkspaceModifyOperation op = new WorkspaceModifyOperation()
        {
            ...
            {
                monitor.beginTask("Diagram Generation process", 11);
                try
                {
                    ...
                    if (manager.getRootModelObject() instanceof DiagramConfiguration)
                    {
                        DiagramConfiguration configuration = (DiagramConfiguration) manager.getRootModelObject();
                        ...
                        if (diagnostic.getSeverity() <= Diagnostic.WARNING)
                        {
                            ...
                            DiagramConfiguratorGenerator generator = new DiagramConfiguratorGenerator(configuration);
                            generatedProject = generator.generate(monitor);
                        }
                        else
                        {
                            GeneratorPlugin.log("Validation problem : a problem occured during the diagram configuration validation.", IStatus.ERROR);
                        }
                    }
                    else
                    {
        ...
        catch (Exception e)
        {
            ...
            GeneratorPlugin.displayDialog(null, "An error occurred during the diagram generation", IStatus.ERROR);
        }
    }
