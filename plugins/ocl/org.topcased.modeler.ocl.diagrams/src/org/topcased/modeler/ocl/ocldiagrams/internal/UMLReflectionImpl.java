 ...
 * </copyright>
 *
 * $Id: UMLReflectionImpl.java,v 1.1 2009/03/19 13:35:49 clecamus Exp $
 */
 ...
 */
public class UMLReflectionImpl implements
        org.eclipse.ocl.utilities.UMLReflection<EPackage, EClassifier, EOperation, EStructuralFeature, EEnumLiteral, EParameter, EObject, CallOperationAction, SendSignalAction, Constraint>
{
    ...
    /**
     * Default implementation just gets the name of the named element's {@link org.eclipse.emf.ecore.EObject#eClass()
     * eClass} or, if it is an {@link ETypedElement}, the name of its type.
     */
    ...
    {
        return (namedElement instanceof ENamedElement) ? getQualifiedName((ENamedElement) namedElement) : null;
    }
    ...
    public EClassifier getOCLType(Object metaElement)
    {
        ...
        else if (metaElement instanceof ETypedElement)
        {
            ...
            {
                result = getOCLCollectionType(result, typedElement.isOrdered(), typedElement.isUnique());
            }
            else
            {
    ...
    {
        return (metaElement instanceof ETypedElement) && isMany((ETypedElement) metaElement);
    }
    ...
    /**
     * Determines whether the specified <code>operation</code> is a query operation. By default, all operations are
     * assumed to be queries.
     * 
    ...
    /**
     * Determines whether the specified <code>eclass</code> is an association class. By default, it is assumed that
     * nothing is an association class because Ecore does not directly support this concept.
     * 
    ...
    /**
     * Obtains the association class that the specified reference represents, as an {@link EClass}, if any. By default,
     * it is assumed that nothing is an association class because Ecore does not directly support this concept.
     * 
    ...
    /**
     * Obtains the ends of the specified association class as a list of {@link EReference}s. By default, it is assumed
     * that nothing is an association class because Ecore does not directly support this concept.
     * 
    ...
    {
        CallOperationAction result = OcldiagramsFactory.eINSTANCE.createCallOperationAction();
    ...
    {
        SendSignalAction result = OcldiagramsFactory.eINSTANCE.createSendSignalAction();
        result.setSignal((BusinessType) signal);
        return result;
    }

    public EClass getSignal(SendSignalAction sendSignalAction)
    {
        return (EClass) sendSignalAction.getSignal();
    }

    public Constraint createConstraint()
    {
        return OcldiagramsFactory.eINSTANCE.createConstraint();
    }
    ...
    {
        return OcldiagramsFactory.eINSTANCE.createExpressionInOCL();
    }
    ...
    /**
     * Gets the qualifiers of an association end. By default, it is assumed that properties never have qualifiers
    ...
    public EClassifier getCommonSuperType(EClassifier type1, EClassifier type2)
    {
        ...
        if (type1 instanceof EClass && type2 instanceof EClass)
        {
            ...
            List<EClass> allSuperAndSelf1 = new ArrayList<EClass>(eClass1.getEAllSuperTypes());
            allSuperAndSelf1.add(eClass1);

            List<EClass> allSuperAndSelf2 = new ArrayList<EClass>(eClass2.getEAllSuperTypes());
     ...
     * 
     * @return either {@link #UNRELATED_TYPE}, {@link #STRICT_SUBTYPE}, {@link #STRICT_SUPERTYPE}, or {@link #SAME_TYPE}
     *         according to the relationship between the types
     */
    ...
    private EClassifier getOCLTypeFor(EDataType dataType)
    {
        ...
        else if (instanceClass == Double.class || instanceClass == double.class || instanceClass == Float.class || instanceClass == float.class)
        {
        ...
        else if (instanceClass == Integer.class || instanceClass == int.class || instanceClass == Long.class || instanceClass == long.class || instanceClass == Short.class
    ...
    /**
     * Obtains the appropriate OCL type for an {@link EClassifier}, if it corresponds to an OCL primitive type.
     * 
    ...
    /**
     * Obtains the appropriate OCL collection type for an {@link EClassifier}, according to the collection's orderedness
     * and uniqueness. The mapping is as follows:
     * <ul>
     ...
     * </ul>
     * <p>
     * Note that the collection's element <code>type</code> is mapped to an OCL primitive type where possible.
     * </p>
     * 
     ...
     */
    EClassifier getOCLCollectionType(EClassifier type, boolean isOrdered, boolean isUnique)
    {
        EClassifier resultType = type;
    ...
    /**
     * Queries whether a prospective <tt>ancestor</tt> really is a container (recursively) of the specified
     * <tt>descendent</tt>. This utility is like the similar {@link EcoreUtil} operation, except that it does not
     ...
     * @return <code>true</code> if the ancestor contains (recursively) the descendent
     */
