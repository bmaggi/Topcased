...
import org.topcased.modeler.ocl.ocldiagrams.OcldiagramsEnvironment;
import org.topcased.modeler.ocl.ocldiagrams.OcldiagramsEnvironmentFactory;
...
{
    private static final String NS_URI = "http://www.eclipse.org/ocldiagrams/stdlib/1.1"; //$NON-NLS-1$
    ...
    public static final EObject OCL_INVALID = stdlibPackage.getEFactoryInstance().create((EClass) stdlibPackage.getEClassifier("Invalid_Class")); //$NON-NLS-1$
    ...
    private static EPackage init()
    {
        ...
        try
        {
            ...
            res = OCLRequirementPlugin.getEcoreResourceFactory().createResource(load.getURI());
            ...
            OCL_TYPE = (EClassifier) EcoreUtil.getObjectByType(stdlibPackage.getEClassifiers(), OcldiagramsPackage.Literals.TYPE_TYPE);

            OCL_SET = (EClassifier) EcoreUtil.getObjectByType(stdlibPackage.getEClassifiers(), OcldiagramsPackage.Literals.SET_TYPE);
            OCL_ORDERED_SET = (EClassifier) EcoreUtil.getObjectByType(stdlibPackage.getEClassifiers(), OcldiagramsPackage.Literals.ORDERED_SET_TYPE);
            OCL_BAG = (EClassifier) EcoreUtil.getObjectByType(stdlibPackage.getEClassifiers(), OcldiagramsPackage.Literals.BAG_TYPE);
            OCL_SEQUENCE = (EClassifier) EcoreUtil.getObjectByType(stdlibPackage.getEClassifiers(), OcldiagramsPackage.Literals.SEQUENCE_TYPE);

            // CLC : OCL_TYPE through OCL_SEQUENCE are null so :
            // Solution for eContents() result method

            OCL_TYPE = stdlibPackage.getEClassifier("OclType"); //$NON-NLS-1$

            OCL_SET = stdlibPackage.getEClassifier("Set(T)"); //$NON-NLS-1$
            OCL_ORDERED_SET = stdlibPackage.getEClassifier("OrderedSet(T)"); //$NON-NLS-1$
            OCL_BAG = stdlibPackage.getEClassifier("Bag(T)"); //$NON-NLS-1$
            OCL_SEQUENCE = stdlibPackage.getEClassifier("Sequence(T)"); //$NON-NLS-1$
    ...
    private static EPackage build()
    {
        ...
        Environment< ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? > old = Environment.Registry.INSTANCE.getEnvironmentFor(OcldiagramsPackage.Literals.ANY_TYPE);
        ...
        Resource.Factory factory = OCLRequirementPlugin.getEcoreResourceFactory();
        ...
        OcldiagramsEnvironment env = (OcldiagramsEnvironment) OcldiagramsEnvironmentFactory.INSTANCE.loadEnvironment(res);
        Environment.Registry.INSTANCE.registerEnvironment(env);

        OCL_ANY = (EClassifier) OcldiagramsFactory.eINSTANCE.createAnyType();
        OCL_ELEMENT = (EClassifier) OcldiagramsFactory.eINSTANCE.createElementType();
        OCL_BOOLEAN = OcldiagramsFactory.eINSTANCE.createPrimitiveType();
        OCL_BOOLEAN.setName(PrimitiveType.BOOLEAN_NAME);
        OCL_INTEGER = OcldiagramsFactory.eINSTANCE.createPrimitiveType();
        OCL_INTEGER.setName(PrimitiveType.INTEGER_NAME);
        OCL_UNLIMITED_NATURAL = OcldiagramsFactory.eINSTANCE.createPrimitiveType();
        OCL_UNLIMITED_NATURAL.setName(PrimitiveType.UNLIMITED_NATURAL_NAME);
        OCL_REAL = OcldiagramsFactory.eINSTANCE.createPrimitiveType();
        OCL_REAL.setName(PrimitiveType.REAL_NAME);
        OCL_STRING = OcldiagramsFactory.eINSTANCE.createPrimitiveType();
        OCL_STRING.setName(PrimitiveType.STRING_NAME);
        OCL_VOID = (EClassifier) OcldiagramsFactory.eINSTANCE.createVoidType();
        OCL_MESSAGE = (EClassifier) OcldiagramsFactory.eINSTANCE.createMessageType();

        INVALID = (EClassifier) OcldiagramsFactory.eINSTANCE.createInvalidType();

        OCL_T = (EClassifier) OcldiagramsFactory.eINSTANCE.createAnyType();
        OCL_T.setName("T"); //$NON-NLS-1$
        OCL_T2 = (EClassifier) OcldiagramsFactory.eINSTANCE.createAnyType();
        ...
        STATE = (EClassifier) OcldiagramsFactory.eINSTANCE.createElementType();
        STATE.setName("State"); //$NON-NLS-1$
        OCL_EXPRESSION = (EClassifier) OcldiagramsFactory.eINSTANCE.createElementType();
        ...
        register(OCL_ANY).getEOperations().addAll(OCLStandardLibraryUtil.createAnyTypeOperations(env));
        register(OCL_VOID).getEOperations().addAll(OCLStandardLibraryUtil.createAnyTypeOperations(env));
        register(INVALID).getEOperations().addAll(OCLStandardLibraryUtil.createAnyTypeOperations(env));
        register(OCL_BOOLEAN).getEOperations().addAll(OCLStandardLibraryUtil.createBooleanOperations(env));
        register(OCL_INTEGER).getEOperations().addAll(OCLStandardLibraryUtil.createIntegerOperations(env));
        register(OCL_UNLIMITED_NATURAL).getEOperations().addAll(OCLStandardLibraryUtil.createUnlimitedNaturalOperations(env));
        register(OCL_REAL).getEOperations().addAll(OCLStandardLibraryUtil.createRealOperations(env));
        register(OCL_STRING).getEOperations().addAll(OCLStandardLibraryUtil.createStringOperations(env));
        register(OCL_TYPE).getEOperations().addAll(OCLStandardLibraryUtil.createTypeTypeOperations(env));
        register(OCL_MESSAGE).getEOperations().addAll(OCLStandardLibraryUtil.createMessageTypeOperations(env));
    ...
    /**
     * Finds the shadow class to contain features defined for the specified OCL <code>type</code>, if it already exists.
     * 
     * @param classifier an Ecore classifier
     * @param pkg the package in which to look for the shadow class
     * 
     * @return the class containing its features, or <code>null</code> if not found
     */
    public static EClass findShadowClass(EClassifier classifier, EPackage pkg)
    {
        for (EClassifier next : pkg.getEClassifiers())
        {
            if (next instanceof EClass)
            {
                ...
                EAnnotation ann = eclass.getEAnnotation(Environment.OCL_NAMESPACE_URI);
    ...
    public static EClassifier getRealClassifier(EClass shadowClass)
    {
        ...
        EAnnotation ann = shadowClass.getEAnnotation(Environment.OCL_NAMESPACE_URI);
    ...
    /**
     * Obtains the existing operations of the specified type, stored in it or in a shadow class. <b>Note</b> that this
     * method returns <code>null</code>, not an empty list, if none are found.
     * 
